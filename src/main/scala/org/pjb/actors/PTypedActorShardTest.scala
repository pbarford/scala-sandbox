...
object PTypedActorShardTest extends App {
  ...
  shardRegion ! ShardingEnvelope("PTypedActor-1", PTypedActor.UpdateData("TESTING"))
  shardRegion ! ShardingEnvelope("PTypedActor-2", PTypedActor.UpdateData("TESTING"))
  shardRegion ! ShardingEnvelope("PTypedActor-1", PTypedActor.UpdateData("TESTING"))

  implicit val timeout: Timeout = 3.seconds
  implicit val scheduler = system.scheduler
  implicit val ec = system.executionContext
  val result: Future[PTypedActor.State] = shardRegion.ask(ref => ShardingEnvelope("PTypedActor-1", PTypedActor.GetState(ref)))
  result.onComplete {
